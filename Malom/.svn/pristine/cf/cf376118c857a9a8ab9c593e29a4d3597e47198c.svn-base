...
public class BabuKezelo
{
    ...
    public void Betoltes(malom main)
    {
        ...
        try
        {
            hiba = SqlKezelo.Instance().GetHiba();
        }
        catch (Exception e)
        {
            hiba = e.toString();
        ...
        if (hiba != "")
        {
            Toast.makeText(main.getApplicationContext(), hiba, Toast.LENGTH_SHORT).show();
        }

        sargaGyozelem = SqlKezelo.Instance().GetSarga();
        dontetlen = SqlKezelo.Instance().GetDontetlen();
        pirosGyozelem = SqlKezelo.Instance().GetPiros();
        ...
        try
        {
            hiba = SqlKezelo.Instance().Log("Program elinditva!");
        }
        catch (Exception e)
        {
            hiba = e.toString();
    ...
    private void ButtonPozicioBeallit(malom main)
    {
        ...
        //következik label
        TextView li2 = (TextView)main.findViewById(R.id.kovetkezikLabel);
        li2.setX(WidthToDp(2, main));
        li2.setY(HeightToDp(-5, main));

        //következik button
        li = main.findViewById(R.id.buttonKovetkezik);
        li.setX(WidthToDp(15, main));
        li.setY(HeightToDp(-7, main));
        SetGombMeret(li, main);

        //Állás label
        li2 = (TextView)main.findViewById(R.id.allasLabel);
        li2.setX(WidthToDp(42, main));
        li2.setY(HeightToDp(80, main));

        //döntetlen button
        li = main.findViewById(R.id.buttonDontetlen);
        li.setX(WidthToDp(48, main));
        li.setY(HeightToDp(-7, main));

        //állás nullázó button
        li = main.findViewById(R.id.buttonNullaz);
        li.setX(WidthToDp(80, main));
        li.setY(HeightToDp(-7, main));

        //Bug report
        li = main.findViewById(R.id.buttonBugReport);
        li.setX(WidthToDp(80, main));
        li.setY(HeightToDp(77, main));
    }
    ...
    private void SetAllas(malom main)
    {
        ...
        if (sargaJatekban < 3)
        {
            pirosGyozelem++;
            frissit = true;
            SqlKezelo.Instance().Gyoztes(2);
            SqlKezelo.Instance().Log("Játék vége, eredmény piros győzött");
        }
        else if (pirosJatekban < 3)
        {
            sargaGyozelem++;
            frissit = true;
            SqlKezelo.Instance().Gyoztes(1);
            SqlKezelo.Instance().Log("Játék vége, eredmény sárga győzött");
        }
    ...
    {
        ((TextView)main.findViewById(R.id.allasLabel)).setText("Állás: " + sargaGyozelem + " - " + dontetlen + " - " + pirosGyozelem);
    }

    public void Nullaz(malom main)
    {
        sargaGyozelem = 0;
        dontetlen = 0;
        pirosGyozelem = 0;
        AllasFrissit(main);
        SqlKezelo.Instance().Nullaz();
        SqlKezelo.Instance().Log("Állás nullázva");
    }

    public void Dontetlen(malom main)
    {
        dontetlen++;
        AllasFrissit(main);
        kovetkezik = Kovetkezik.Sarga;
        ...
        SqlKezelo.Instance().Gyoztes(0);
        SqlKezelo.Instance().Log("Játék vége, eredmény döntetlen");
    }
